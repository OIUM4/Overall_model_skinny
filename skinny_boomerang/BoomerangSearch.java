...
public class BoomerangSearch implements Callable<Integer> {
  ...
  @Option(names = {"-timeLimit","-tl"}, defaultValue = "0", description = "Time limit in seconds for Gurobi solver (0 = no limit). Default is ${DEFAULT-VALUE}")
  ...
  @Option(names = {"-step1MinObjectiveValue","-s1minobj"}, defaultValue = "-1", description = "Value of objective (may not be optimal). Default search for optimal solutions. Seems to greatly increase the running time, preferably you should also generate the optimal solutions")
  private int step1MinObjectiveValue;
  @Option(names = {"-step1MaxObjectiveValue","-s1maxobj"}, defaultValue = "-1", description = "Upper bound for Step1 objective. If set, Step1 enters target-search mode: add objective <= value, disable solution pool enumeration, stop after the first feasible solution at or below this bound. Use -s1maxobj=111 to search for a solution better than known objective 112. -1 = no upper bound. Default is ${DEFAULT-VALUE}")
  ...
  public Integer call() {
    ...
    if (step2input == null) {
      ...
      if (step1Solutions.isEmpty()) {
        System.out.println("No Step1 solutions to save. Exiting.");
        System.exit(1);
        return 1;
      }
  ...
  private List<Step1Solution> getStep1Solutions() {
    try {
      ...
      if (verbose) {
        ...
        if (step1MaxObjectiveValue != -1) {
          System.out.println("Step1 target-search mode enabled: objective <= " + step1MaxObjectiveValue + " (-s1maxobj)");
        }
      }
      List<Step1Solution> step1Solutions = step1.solve(nbStep1Sols, nonOptimalStep1Sols, step1MinObjectiveValue, step1MaxObjectiveValue, nbThreads, timeLimit);
      if (step1Solutions.isEmpty()) {
        if (verbose) {
          System.out.println("No solution found (time limit reached or infeasible).");
        } else {
  ...
  private List<Step2Solution> getStep2Solutions(List<Step1Solution> step1Solutions) {
    ...
    for (int step1Index = 0; step1Index < step1Solutions.size(); step1Index++) {
      Step1Solution step1Solution = step1Solutions.get(step1Index);
      int step2Regime = step1Solution.regime;
      List<Step2Solution> perStep1Solutions =
        (step2Regime == 0)
          ? Step2.step2SK(step1Solution, sboxTables, step2Regime, nbStep2Sols, clusterGap, verbose)
          : Step2.optimizeEnumerateCluster(step1Solution, sboxTables, step2Regime, nbStep2Sols, clusterGap, verbose);
      for (Step2Solution step2Solution : perStep1Solutions) {
        step2Solution.step1SolutionIndex = step1Index;
        step2Solution.step1Objective = step1Solution.objective;
        step2Solutions.add(step2Solution);
      }
    }
    if (verbose) {
