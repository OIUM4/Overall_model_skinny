...
public class Step2 {
  ...
  /** Eb-last round: SBox input differences (null for inactive cells). */
  private final IntVar[][] dXupper_EbLast;
  /** Eb-last round: SBox output differences (green region in the figure). */
  private final IntVar[][] dSBupper_EbLast;
  /** Eb-last round: post-tweakey state for rows 0-1 (null when no TK or SK regime). */
  private final IntVar[][] dYupper_EbLast;
  /** Eb-last round: concrete TK difference derived from Step2 key expansion. */
  private final IntVar[][] dTKupper_EbLast;
  ...
  public Step2(Step1Solution step1, SboxTables sboxTables, final int regime) {
    ...
    // -------- Eb-last round: initialize variables if nExtbRounds > 0 --------
    if (nExtbRounds > 0) {
      dXupper_EbLast  = new IntVar[4][4];
      dSBupper_EbLast = new IntVar[4][4];
      factory.initializeEbLastRound(dXupper_EbLast, dSBupper_EbLast,
                                     step1.DXuppExt[nExtbRounds - 1]);
      if (regime != 0 && step1.DSTKuppExt != null) {
        dTKupper_EbLast = dTKupper.getBeforeFirstRoundTweakey(step1.DSTKuppExt[nExtbRounds - 1]);
        dYupper_EbLast = factory.initializeAndPostTweakeyEbLast(
            dSBupper_EbLast,
            step1.DXuppExt[nExtbRounds - 1],
            dTKupper_EbLast,
            step1.DSTKuppExt[nExtbRounds - 1]);
      } else {
        dTKupper_EbLast = null;
        dYupper_EbLast = null;
      }
    } else {
      dXupper_EbLast  = null;
      dSBupper_EbLast = null;
      dYupper_EbLast  = null;
      dTKupper_EbLast = null;
    }
    ...
    // -------- Eb-last → Ed-first connection constraints --------
    if (nExtbRounds > 0 && dSBupper_EbLast != null) {
      if (regime == 0) {
        factory.connectEbLastToEdFirstSK(dSBupper_EbLast, dXupper,
            step1.DXupper, step1.freeXupper, step1.DXuppExt[nExtbRounds - 1]);
      } else {
        factory.connectEbLastToEdFirstTK(dSBupper_EbLast, dYupper_EbLast, dXupper,
            step1.DXupper, step1.freeXupper,
            step1.DXuppExt[nExtbRounds - 1], step1.DSTKuppExt[nExtbRounds - 1]);
      }
    }
  ...
  private Step2Solution getSolution() {
    ...
    // -------- Extract Eb-last round concrete values --------
    int[][] dXupper_EbLastValue  = null;
    int[][] dSBupper_EbLastValue = null;
    int[][] dTKupper_EbLastValue = null;
    if (nExtbRounds > 0 && dXupper_EbLast != null) {
      dXupper_EbLastValue  = new int[4][4];
      dSBupper_EbLastValue = new int[4][4];
      for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++) {
          if (dXupper_EbLast[i][j]  != null) dXupper_EbLastValue[i][j]  = dXupper_EbLast[i][j].getValue();
          if (dSBupper_EbLast[i][j] != null) dSBupper_EbLastValue[i][j] = dSBupper_EbLast[i][j].getValue();
        }
      // Extract concrete TK difference for the last Eb round from the key schedule.
      if (regime != 0 && dTKupper_EbLast != null) {
        dTKupper_EbLastValue = new int[2][4];
        for (int i = 0; i < 2; i++)
          for (int j = 0; j < 4; j++)
            if (dTKupper_EbLast[i][j] != null)
              dTKupper_EbLastValue[i][j] = dTKupper_EbLast[i][j].getValue();
      }
    }
    return new Step2Solution(nbRounds, nExtbRounds, nExtfRounds, -(double) objective.getValue()/Step2Factory.probaFactor,  dXupperValue, dSBupperValue, dYupperValue, (regime == 0) ? null : dTKupper.getValue(), dXlowerValue, dSBlowerValue, dYlowerValue, (regime == 0) ? null : dTKlower.getValue(), probaValue, dXupper_EbLastValue, dSBupper_EbLastValue, dTKupper_EbLastValue);
  }
