...
public class SolutionToTikz implements Callable<Integer> {
  ...
  @Option(names = {"-step2best","-s2b"}, description = "For Step2: pick best probability (within Step1 index if -s1sol is set)")
  private boolean step2best;
  @Option(names = {"-step1SolForStep2","-s1sol"}, defaultValue = "-1", description = "Step1 solution index whose Step2 overlay to use (-1 = same as -sol)")
  private int step1SolForStep2;
  ...
  public Integer call() {
    ...
    if (step2input != null) {
      int step1Index = (step1SolForStep2 >= 0) ? step1SolForStep2 : solNumber;
      tikz = new Step2SolutionToTikz(step2input, step1Index, solNumber, step2best).generate();
    }
